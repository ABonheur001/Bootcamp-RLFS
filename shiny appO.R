...
tablee<-data.frame(
  ...
  percenta <- c(41.9,39.5,37.4,40.5,47.7,46.8))

output$agriculture <- renderHighchart({
    highchart() %>%
      hc_chart(type = "column") %>%
      hc_title(text = "Highcharter Column Chart Example") %>%
      hc_xAxis(categories = tablee$years) %>%
      hc_yAxis(title = list(text = "Percentages")) %>%
      hc_series(list(name = "years", data = tablee$percenta))
}) 
    #services
dtable <- data.frame(
    years <- c("2017",'2018',"2019","2020",'2021',"2022"),
    Percent<- c(41.5,41.7,43.9,39.1,35.1,35.9))

output$serv <- renderHighchart({
    highchart() %>%
      hc_chart(type = "column") %>%
      hc_title(text = "Highcharter Column Chart Example") %>%
      hc_xAxis(categories = dtable$years) %>%
      hc_yAxis(title = list(text = "Percentages")) %>%
      hc_series(list(name = "years", data = dtable$Percent))
  })
  ...
  output$indu <- renderHighchart({
             highchart() %>%
                   hc_chart(type = "column") %>%
                   hc_title(text = "Highcharter Column Chart Example") %>%
                   hc_xAxis(categories = tab$years) %>%
                   hc_yAxis(title = list(text = "Percentages")) %>%
                   hc_series(list(name = "years", data = tab$percentages))
        })
